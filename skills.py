...
def all_odd(number_list):
    ...
    """
    # list comprehension solution, equivalent for-loop follows
    odd_list = [item for item in number_list if item % 2 == 1]
...
def long_words(word_list):
    ...
    """
    # list comprehension solution; for loop solution commented below
    four_char = [word for word in word_list if len(word) > 4]
...
def sum_numbers(number_list):
    ...
    """
    val = 0  # for empty list case
    if number_list:
        val = reduce(lambda x, y: x+y, number_list)
    return val
...
def average(number_list):
    ...
    try:
        ...
        return avg
    except TypeError:  # handle the empty list exception
        return None
